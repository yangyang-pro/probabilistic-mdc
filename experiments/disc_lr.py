...
if __name__ == '__main__':
    ...
    base_clf = SGDClassifier(loss='log_loss', n_jobs=-1)
    ...
    for i, (train_indices, test_indices) in enumerate(kf.split(data_loader.labels)):
        ...
        encoder.fit(train_data_loader.discrete_features)
        train_disc_features = encoder.transform(train_data_loader.discrete_features)
        test_disc_features = encoder.transform(test_data_loader.discrete_features)
        train_features = np.hstack((train_data_loader.continuous_features, train_disc_features))
        test_features = np.hstack((test_data_loader.continuous_features, test_disc_features))

        cc_encoder = OneHotEncoder(sparse=False, handle_unknown='ignore')
        cc_encoder.fit(cc_train_data_loader.discrete_features)
        cc_train_disc_features = cc_encoder.transform(cc_train_data_loader.discrete_features)
        cc_val_disc_features = cc_encoder.transform(cc_val_data_loader.discrete_features)
        cc_train_features = np.hstack((cc_train_data_loader.continuous_features, cc_train_disc_features))
        cc_val_features = np.hstack((cc_val_data_loader.continuous_features, cc_val_disc_features))

        # BR
        br = BinaryRelevance(classifier=base_clf)
        br.fit(X=train_features, y=train_data_loader.labels)
        pred = br.predict(X=test_features)
        ...
        cp.fit(X=train_features, y=train_data_loader.labels)
        pred = cp.predict(X=test_features)
        ...
        for j in range(n_chains):
            ...
            cc.fit(X=cc_train_features, y=cc_train_data_loader.labels)
            pred = cc.predict(X=cc_val_features)
        ...
        hl_cc.fit(train_features, train_data_loader.labels)
        zo_cc.fit(train_features, train_data_loader.labels)
        pred_hl = hl_cc.predict(test_features)
        pred_zo = zo_cc.predict(test_features)
